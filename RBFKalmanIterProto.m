function [AveCorrect, AveIter, AveCPU] = RBFKalmanIterProto(xall, yall, cmin, cmax, gamma, m, epsilon, P0, Q0, R0)

% [AveCorrect, AveIter, AveCPU] = RBFKalmanIterProto(xall, yall, cmin, cmax, gamma, m, eta, epsilon)
% Test a Radial Basis Function network trained with a Kalman filter.
% This function iterates on the number of prototypes in the RBF network.
% for each prototype count, the RBF is tested by calling the RBFKalmanIter function.
%
% INPUTS
...
% P0 = initial setting of estimate covariance matrix (200?)
% Q0 = initial setting of state covariance matrix (40?)
% R0 = initial setting of measurement covariance matrix (50?)
%
% OUTPUTS
...
for c = cmin : cmax
   ...
   disp(' ');
   [Correct, Iter, CPU] = RBFKalmanIter(xall, yall, c, gamma, m, epsilon, P0, Q0, R0);
   AveCPU(c-cmin+1) = CPU;
